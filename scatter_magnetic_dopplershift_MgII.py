...
from scatter_AB import scatterAB,scatterAB_p
...
for file in files[0:1]:
    ...
    s_hmi = mhmi_sub.data.shape
    s_dopplershift = m_doppshift_av_sub.data.shape
    
    # reduce hmi array shape by z
    z = (s_dopplershift[0]/s_hmi[0],s_dopplershift[1]/s_hmi[1])
    mhmi_zoomed_data = zoom(mhmi_sub.data,z)
    # print(s_hmi)
    # print(mhmi_zoomed_data.data.shape)
    # print(s_dopplershift)
    
    im_scatter = scatterAB_p(mhmi_zoomed_data,m_doppshift_av_sub.data,ax,ext_y=ext_y)
    plt.colorbar(im_scatter,label='Solar Y (arcsec)')

    fig1 = plt.figure()
    ax1 = fig1.add_subplot(1,1,1)
    
    ax1.imshow(m_doppshift_av_sub.data,origin='lower',vmin=-5,vmax=5,extent=ext_raster,cmap='RdBu_r')
    ax.set_xlabel("Dopplershift (MgII)")
    ax.set_ylabel("$B_{los}$")
